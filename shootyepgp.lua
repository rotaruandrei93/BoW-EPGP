...
function sepgp:buildMenu()
  if not (options) then
    options = {
    type = "group",
    desc = string.format("ShootyEPGP [Blades of Wrynn] v%s", sepgp._versionString or "?"),
    handler = self,
    args = { }
    }
    options.args["version"] = {
      ...
      order = 1,
      func = function() sepgp:defaultPrint(string.format("ShootyEPGP [Blades of Wrynn] v%s", sepgp._versionString or "?")) end,
    }
    options.args["ep"] = {
      type = "group",
...
function sepgp:delayedInit()
  ...
  if IsGuildLeader() and ( (sepgp_dbver == nil) or (major_ver > sepgp_dbver) ) then
    ...
    if sepgp[migrationName] then
      ...
      self:defaultPrint(string.format("ShootyEPGP: No migration needed (%s), updating dbver.", migrationName))
...
function sepgp:adminSay(msg)
  ...
  SendChatMessage(string.format("shootyepgp: %s", cleanMsg), "OFFICER")
end
...
-- Survives /reload and logout. Check WTF/Account/<acct>/SavedVariables/ShootyEPGP.lua
