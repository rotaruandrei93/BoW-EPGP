...
function sepgp:givename_gp(getname,gp) -- assigns gp to a single character
  ...
  local itemInfo = ""
  if sepgp.bid_item and sepgp.bid_item.name and sepgp.bid_item.name ~= "" then
    itemInfo = " for " .. sepgp.bid_item.name
  end
  self:adminSay(string.format("[EPGP-AUDIT] %s awarded %d GP to %s%s%s (was %d, now %d)", UnitName("player"), gp, getname, postfix, itemInfo, oldgp, math.max(sepgp.VARS.basegp,newgp)))
  self:addToLog(msg)
